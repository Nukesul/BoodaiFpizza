...
class BranchAdmin(CustomAdminMixin, admin.ModelAdmin):
    list_display = ('name', 'address', 'phone', 'pizza_count')
    ...
    def get_queryset(self, request):
        ...
        return queryset.annotate(pizza_count=Count('pizza'))
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        form.base_fields['name'].label = "Название филиала"
        form.base_fields['address'].label = "Адрес"
        form.base_fields['phone'].label = "Телефон"
        return form
...
class PizzaAdmin(CustomAdminMixin, admin.ModelAdmin):
    list_display = ('name', 'category', 'branch', 'base_price', 'stock', 'display_image', 'is_available', 'discount', 'final_price')
    list_filter = ('category', 'branch', 'stock', 'discount')
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'category', 'branch', 'base_price', 'description', 'image', 'stock', 'discount')
        }),
    )
    ordering = ('name',)
    ...
    def export_to_csv(self, request, queryset):
        ...
        writer.writerow(['ID', 'Название', 'Категория', 'Филиал', 'Базовая цена', 'Запас', 'Скидка', 'Итоговая цена'])
        for pizza in queryset:
            ...
            writer.writerow([
                ...
                pizza.name,
                pizza.category.name,
                pizza.branch.name if pizza.branch else 'Нет филиала',
                pizza.base_price,
                pizza.stock,
                pizza.discount,
                final_price
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        form.base_fields['branch'].label = "Филиал"
...
class OrderAdmin(CustomAdminMixin, admin.ModelAdmin):
    list_display = ('id', 'customer_name', 'total', 'delivery', 'branch', 'status', 'created_at', 'item_count', 'view_items', 'promo_code')
    list_filter = ('delivery', 'branch', 'status', 'created_at')
    search_fields = ('customer_name', 'address', 'comment', 'promo_code')
    readonly_fields = ('created_at',)
    actions = ['mark_as_express', 'mark_as_shipped', 'mark_as_delivered', 'cancel_orders', 'export_to_csv']
    fields = ('customer_name', 'address', 'branch', 'delivery', 'comment', 'total', 'status', 'created_at', 'items', 'promo_code')
    ...
    def export_to_csv(self, request, queryset):
        ...
        writer = csv.writer(response)
        writer.writerow(['ID', 'Клиент', 'Итого', 'Тип доставки', 'Филиал', 'Статус', 'Дата создания', 'Пиццы', 'Промокод'])
        for order in queryset:
            writer.writerow([
                ...
                order.total,
                order.delivery,
                order.branch.name if order.branch else 'Нет филиала',
                order.status,
                order.created_at,
                order.items,
                order.promo_code or 'Нет'
            ])
        return response
    export_to_csv.short_description = "Экспортировать в CSV"
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        form.base_fields['address'].label = "Адрес"
        form.base_fields['branch'].label = "Филиал (для самовывоза)"
        ...
        form.base_fields['promo_code'].label = "Промокод"
        return form